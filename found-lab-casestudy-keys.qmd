...
There are different types of joins used to combine information from two
or more tables based on a related column between them. See below:
...
Let's take a look at some mock data first to see how different joins
work. We will need to create data frames first to be able to experiment
with joining different data:
...
class. 2. Save the new value to `student_id` variable. 3. Finally, press
the green arrow head to run the code.
...
❓ Based on the summary statistics, what insights can you gather about
the distribution and characteristics of the variables in the dataset?
Are there any trends or patterns that stand out to you?
