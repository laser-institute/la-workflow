---
title: "Narrated: Foundations Case Study - Student Version" 
...
+------------------+-----------------------------------------------+
| `e n r           | reason student decided to take the course     |
| ol lment_reason` |                                               |
+------------------+-----------------------------------------------+
| `e n r           | ap pr ove/enrolled, dropped, withdrawn        |
| ol lment_status` |                                               |
+------------------+-----------------------------------------------+
...
| Variable              | Description                              |
+=======================+==========================================+
| `t o                  | available points for the course          |
| tal_p oints_possible` |                                          |
+-----------------------+------------------------------------------+
| `                     | stud \| ent earned for the entire course |
| total _points_earned` | \|                                       |
+-----------------------+------------------------------------------+
...
![](img/global_environment.png){width="50%"}
...
If you want ot look for NA's in all your columns you can use dplyr along
with the is.na() function to check for NA's in your columns
...
2.  add the same column to the sum(is.na()) function

```{r eval = FALSE}
data_to_explore %>%
  select(#add column here) %>%  # Fill in the column you want to look at
  summarize(na_count = sum(is.na(#add column here)))  # count NA values in the "semester" column
```
...
Use the car package For more information {[car
package](https://cran.r-project.org/web/packages/car/index.html)}

##### **👉 Your Turn** **⤵**

We will need to add the car package to find the VIF of our model

-   Use the `vif()` function and add the model `m1`
...
Quarto "data product" designed to illustrate key findings. Feel free to
use the template in the lab 4 folder.
