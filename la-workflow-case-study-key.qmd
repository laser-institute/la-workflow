...
**Variables:** - `student_id` = students id at institution course_id = abbreviation for \`course, course number, semester
...
gender = male/female/NA enrollment_reason = why student took course enrollment_status = approve/enrolled, dropped, withdrawn time_spent = Time spent in hours for entire course
...
1.  First, add a function to the code (to inspect the data (your choice).
2.  Second, press the green arrow head to run the code.

```{r}
#load log file from data folder
time_spent <- read_csv("/cloud/project/Foundation_R/module_1/data/log-data.csv", show_col_types = FALSE)
...
**Variables:** - `total_points_possible` = available points for the course - `total_points_earned` = total points student earned for the entire course
...
#(add code below)
gradebook <- read_csv("/cloud/project/Foundation_R/module_1/data/gradebook-summary.csv",show_col_types = FALSE)
...
Variables: - `int` = student science interest - `tv` = how many hours a day student watches tv - `Q1-Q10` = survey questions
...
#(add code below)
survey <- read_csv("/cloud/project/Foundation_R/module_1/data/survey.csv",show_col_types = FALSE)
...
Notice what you are seing between the data frames. Do the variables have the same classes, do you notice any discrepancies that may need to be fixed in wrangling?
...
Let's change that. For this, we will use `mutate()` function. The `mutate()` function in R programming is used to create new variables.
...
Fortunately the {[janitor](https://garthtarr.github.io/meatR/janitor.html)} package has simple functions for examining and cleaning dirty data. It was built with beginning and intermediate R users in mind and is optimized for user-friendliness. There is also a handy function called `clean_names()` in the {janitor} package for standardizing variable names.
...
2.  Second clean the columns by adding the `survey` object to the `clean_names()` functon and saving it to the `survey`object.
...
There are different types of joins used to combine information from two or more tables based on a related column between them. See below:

![](Foundation_R/joins.png)

Image source: <https://medium.com/>@imanjokko/data-analysis-in-r-series-vi-joining-data-using-dplyr-fc0a83f0f064

Let's take a look at some mock data first to see how different joins work. We will need to create data frames first to be able to experiment with joining different data:
...
❓ What correlations in the columns do you notice?

Run the code and see what results are returned with each type of join:
...
❓ Which join do you think is best to use for our case study? Why?
...
When we are combining `gradebook1` and `time_spent` datasets, we should identify column names. In this case, we will use the following variables for the match: - `student_id,` - `subject,` - `semester,` - `section`
...
You need to: 1. Use `full join` function to join `joined_data` with `survey` dataset with the following variables: - `student_id,` - `subject,` - `semester,` - `section` 2. Save to a new object called `data_to_explore`. 3. Inspect the data by clicking the green arrow head
...
You need to: 1. First, use the `mutate` function and `as.character()` function to change `student_id` variable from numeric to character class. 2. Save the new value to `student_id` variable. 3. Finally, press the green arrow head to run the code.
...
#### 2b-4 Summary statistics

Let us explore the finalized dataset.
...
❓ Based on the summary statistics, what insights can you gather about the distribution and characteristics of the variables in the dataset? Are there any trends or patterns that stand out to you?

#### 2b-4 Use `filter()` function

We can identify students at risk of failing the course using the filter function:

```{r}
#Filter students with lower grades
at_risk_students <- filter(data_to_explore,proportion_earned<70)
...
write_csv(data_to_explore, "/cloud/project/Foundation_R/module_1/data/data_to_explore.csv")
...
3.  Third, use the `group_by()` function with `subject` variable - then `skim()`
...
To answer the following research question we can add a scale layer: \#### ❓ What can we notice about gender?
...
### 🛑 Stop here. Congratulations you finished the third part of the case
